...
class auth_plugin_earlychildhood extends auth_plugin_base {
    ...
    public function loginpage_hook(){
        ...
        $earlychildhood_host = "http://primerainfanciasantiagodecali.org/app/web/oauth/v2/token";
        $user = "admin";
        $password = "admin1234";

        $data = new stdClass();
        $data->grant_type = $grant_type;
        $data->client_kety = $client_key;
        ...
        if( (strlen($earlychildhood_host) > 0) && (strlen($client_key) > 0) && (strlen($client_secret) > 0) ){
            $file = "/var/www/html/moodle35/auth/earlychildhood/file.txt";
            $fp = fopen($file, "w");
            // $url = 'http://api.flickr.com/services/xmlrpc/';
            $curl_request = curl_init($earlychildhood_host);

            curl_setopt($curl_request, CURLOPT_POST, 1);
            curl_setopt($curl_request, CURLOPT_POSTFIELDS, $data);
            curl_setopt($curl_request, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl_request, CURLOPT_FILE, $fp);
